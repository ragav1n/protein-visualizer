...
# def detect_pockets(mol, grid=1.0, sigma=1.2, min_size=10):
#     atoms = mol["atoms"]
...
#     # voxel grid
#     grid_x = np.arange(mins[0], maxs[0], grid)
#     grid_y = np.arange(mins[1], maxs[1], grid)
#     grid_z = np.arange(mins[2], maxs[2], grid)
...
#     # fill occupancy
#     for x, y, z in coords:
#         ix = int((x - mins[0]) / grid)
#         iy = int((y - mins[1]) / grid)
#         iz = int((z - mins[2]) / grid)
#         if 0 <= ix < grid_shape[0] and 0 <= iy < grid_shape[1] and 0 <= iz < grid_shape[2]:
#             voxels[ix, iy, iz] = 1

#     # smooth surface → cavities become local minima
#     smoothed = gaussian_filter(voxels, sigma)

#     # cavities ≈ empty regions next to filled regions
#     cavity_mask = (smoothed < 0.05)

#     # label cavities
#     labeled, num = label(cavity_mask)

#     pockets = []
#     for i in range(1, num + 1):
#         pocket_indices = np.argwhere(labeled == i)
#         if len(pocket_indices) < min_size:
#             continue

#         # compute pocket center
#         center = pocket_indices.mean(axis=0)
#         center_world = [
#             mins[0] + center[0] * grid,
#             mins[1] + center[1] * grid,
#             mins[2] + center[2] * grid,
#         ]
